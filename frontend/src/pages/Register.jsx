...
export default function Register(){
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gray-100">
      <form onSubmit={submit} className="card p-8 w-[420px]">
        <h1 className="text-2xl font-bold mb-2">Criar conta</h1>
        {error && <div className="bg-red-100 text-red-700 px-3 py-2 rounded mb-3">{error}</div>}
        <input className="w-full bg-gray-100 rounded px-3 py-2 mb-3" placeholder="Nome" value={name} onChange={e=>setName(e.target.value)} />
        ...
        <button className="w-full bg-gray-900 text-white rounded px-3 py-2">Registrar</button>
        <div className="text-sm text-center mt-4">Já tem conta? <Link className="text-blue-600" to="/login">Entrar</Link></div>
      </form>
    </div>
  )
}
